...
class DatacenterGrid:
    ...
    def get_data_center_carbon_intensity(self, load_index: int) -> float:
        ...
        for elem, _ in GENERATION_SOURCES:
            ...
            carbon_contributions = carbon_contributions = E_distribution_mw[connected_bus_df.values, target_bus_df_id].clip(lower=0) * carbon_intensities.values
    ...
    def get_grid_carbon_emissions_rate(self) -> float:
        """
    ...
    def simplify_grid(self) -> None:
        ...
        # )
        renewable_source_ids = self._net.gen.index[
            self._net.gen[self._source_label].isin(RENEWABLE_SOURCE_TYPES)
        ].tolist()
        self.process_grid.sync_generator_costs(
            net=self._net, renewable_sources=renewable_source_ids
        )
    ...
    def run_flow(self, weather_index: int) -> None:
        ...
        solar_power_capcity = self._net.gen.loc[solar_mask, self._power_limit_label]
        ...
        self._net.gen.loc[solar_mask, "max_p_mw"] = solar_power_capcity * (1 - cloudcover / 100)
    ...
    def get_gross_load_demand(self) -> float:
        ...
        return self._net["load"].loc[:, "p_mw"].sum()
    ...
    def __initialize_energy_source_labels(self) -> str:
        ...
        for elem, _ in GENERATION_SOURCES:
            for id in self._net[elem].index.values:
                # We store the energy source type in a custom label inside pandapower net
                self._net[elem].loc[id, source_label] = _DEFAULT_ENERGY_SOURCE
        return source_label
